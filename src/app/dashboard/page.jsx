...
export default function Dashboard() {
  ...
  const stateChartData = {
    labels: data.states && data.states.slice(0, 10).map((s) => s._id || "Unknown"),
    datasets: [
      {
        label: "Visitors",
        data: data.states && data.states.slice(0, 10).map((s) => s.count),
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Charts Grid - Countries and States */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
            <h2 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
              <Globe size={20} className="text-yellow-500" /> Visitors by Country
            </h2>
            <div className="h-80">
          ...
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
            <h2 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
              <MapPin size={20} className="text-purple-500" /> Visitors by State/Region
            </h2>
            <div className="h-80">
        ...
        {/* Charts Grid - Cities and Traffic */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
            <h2 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
              <MapPin size={20} className="text-blue-500" /> Top Cities
            </h2>
            <div className="h-80">
          ...
          <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
            <h2 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
              <Activity size={20} className="text-green-500" /> Traffic History (7 Days)
            </h2>
            <div className="h-80">
        ...
        <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800">
          <h2 className="text-xl font-semibold mb-4 text-white">
            📍 Visitor Details (Country, State, City)
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-700">
                  <th className="text-left py-3 px-4 text-zinc-400">Location</th>
                  <th className="text-left py-3 px-4 text-zinc-400">Visitors</th>
                </tr>
              </thead>
              <tbody>
                {data.locations && data.locations.slice(0, 15).map((loc, idx) => {
                  const country = loc._id.country || "Unknown";
                  const state = loc._id.region || "-";
                  const city = loc._id.city || "-";
                  const locationString = `${country}, ${state}, ${city}`;
                  ...
                  return (
                    <tr key={idx} className="border-b border-zinc-800 hover:bg-zinc-800/50">
                      ...
                      <td className="py-3 px-4 font-semibold text-yellow-500">{loc.count}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <p className="text-xs text-zinc-500 mt-4">
            💡 Shows visitor distribution by Country, State, and City. GPS coordinates appear when visitors grant location permission.
          </p>
        </div>
      </div>
    </div>
